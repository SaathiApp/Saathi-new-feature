...
def history():
    if request.method == "POST":
        # Get selected chat ID from the form
        chat_id = request.form.get("chat_id")
        if chat_id:
            return redirect(url_for("continue_chat", chat_id=chat_id))
        else:
            return "No chat selected.", 400

    # Fetch all chats from MongoDB
    chats = shared_chats_collection.find()
    return render_template("history.html", chats=chats)


@app.route("/continue_chat/<chat_id>", methods=["GET", "POST"])
def continue_chat(chat_id):
    # Fetch chat history from MongoDB
    chat_data = shared_chats_collection.find_one({"_id": chat_id})
    if not chat_data:
    ...
    if request.method == "POST":
        ...
        # Continue chat using the saved session
        session_id = session.get('session_id')
        if session_id and session_id in chat_sessions:
            chat = chat_sessions[session_id]
        else:
            ...
            chat = model.start_chat(history=[
                {"role": "user", "parts": [message]} if i % 2 == 0 else {"role": "model", "parts": [message]}
        ...
        # Save updated chat history in MongoDB
        shared_chats_collection.update_one(
            {"_id": chat_id},
            {"$set": {"chat_history": chat_history}}
        )

    return render_template("continue_chat.html", chat_history=chat_history,chat_id=chat_id)
